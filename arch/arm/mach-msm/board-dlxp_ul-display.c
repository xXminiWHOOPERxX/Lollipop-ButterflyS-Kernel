...
static int mipi_dsi_panel_power(int on)
{
	...
	if (!dsi_power_on) {
		...
		gpio_tlmm_config(GPIO_CFG(LCD_RSTz, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	...
	if (on) {
		if (!first_init) {
			...
			gpio_set_value(LCD_RSTz, 0);
			msleep(10);
			gpio_set_value(LCD_RSTz, 1);
...
static struct dsi_buf dlxp_ul_panel_tx_buf;
static struct dsi_buf dlxp_ul_panel_rx_buf;
...
static char CABC_min[3] = {0x5E, 0x00, 0x23};
...
static int dlxp_ul_lcd_on(struct platform_device *pdev)
{
	...
	if(!first_init) {
		if (mipi->mode == DSI_VIDEO_MODE) {
			mipi_dsi_cmds_tx(&dlxp_ul_panel_tx_buf, video_on_cmds,
				video_on_cmds_count);
		}
	}
	first_init = 0;
...
static void dlxp_ul_display_on(struct msm_fb_data_type *mfd)
{
...
static void dlxp_ul_display_off(struct msm_fb_data_type *mfd)
{
...
static struct msm_fb_panel_data dlxp_ul_panel_data = {
	...
	.display_on = dlxp_ul_display_on,
	.display_off = dlxp_ul_display_off,
};

static struct msm_panel_info pinfo;
static int ch_used[3] = {0};

static int mipi_dlxp_ul_device_register(struct msm_panel_info *pinfo,
...
static int __init dlxp_ul_panel_init(void)
{
	...
	mipi_dsi_buf_alloc(&dlxp_ul_panel_tx_buf, DSI_BUF_SIZE);
	mipi_dsi_buf_alloc(&dlxp_ul_panel_rx_buf, DSI_BUF_SIZE);
