 ...
 *
 * Sweep2wake, Doubletap2wake & Wake Gestures with Pocket Detection for HTC One
 * Copyright (C) 2013,2014 Aaron Segaert aka flar2 (asegaert at gmail.com)  
 *
...
static ssize_t synaptics_wake_gestures_dump(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
...
static DEVICE_ATTR(wake_gestures, 0666,
	synaptics_wake_gestures_show, synaptics_wake_gestures_dump);
...
{
	int prevy = 0, nexty = 0;
	if (!scr_suspended || (!gestures_switch && !(s2w_switch & SWEEP_UP) && !(s2w_switch & SWEEP_DOWN))){
		return;
	}
...
static void sweep2wake_horiz_func(int x, int y)
{
	...
	if (!scr_suspended || !gestures_switch) 
		return;
	...
	//sweep right
	if (firstx < 810 && y < 2700) {
		prevx = firstx;
		nextx = prevx + 180;
		if (barrierx[0] == true || (x > prevx && x < nextx && y < 2700 )) {
			prevx = nextx;
			nextx += 200;
			barrierx[0] = true;
			if (barrierx[1] == true || (x > prevx && x < nextx && y < 2700 )) {
				prevx = nextx;
				barrierx[1] = true;
				if (x > prevx && y < 2700 ) {
					if (x > (nextx + 180)) {
...
{
	if ((x > 0 && x < 150) || x > 1470 || y > 2700 || 
		(!gestures_switch && dt2w_switch == 1 && (y > 0 && y < 2100))) {
		reset_dt2w();
		return;
	}
...
static void synaptics_ts_finger_func(struct synaptics_ts_data *ts)
{
	...
	if (ts->finger_count <= 0) {
		if (scr_suspended) {
			if (gestures_switch || dt2w_switch) { 
...
static void synaptics_ts_status_func(struct synaptics_ts_data *ts)
{
	...
	if (ret < 0) {
		...
		if (data == 1) {
			...
			if (scr_suspended) {
				if (gestures_switch || dt2w_switch) { 
					printk(KERN_INFO "[TP] dt2w in status_func\n");
